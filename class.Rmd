...
population %>% 
  names(sort(.)) %>%
  factor(cities, levels=.) -> cities
#%>%是把population放到names()的input裡，而非sort()，但names()的input位置(只能放一個物件)已被sort佔走
```
但可寫成
```{r}
population %>% 
...
```{r}
map(dataList, ~is.numeric(.x)) -> results #結果會是list
unlist(results) -> pick_numeric #換成要/不要向量
...
grade <-  cut(grade,c(-1,69,79,89,100)) 
